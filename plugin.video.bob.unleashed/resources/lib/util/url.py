...
def __replace_gif(url):
    ...
    if not url.endswith(".gif"):
        ...
        dest_folder = os.path.join(xbmc.translatePath(
            xbmcaddon.Addon().getAddonInfo("profile")), "artcache")
        ...
        if not xbmcvfs.exists(dest):
            ...
            if response.status_code == 200:
                ...
                if os.path.getsize(dest) == 0:
                    xbmc.log("0 size gif: " + repr(dest),xbmc.LOGNOTICE)
                    os.remove(dest)
                    return None
                else:
                    xbmc.log("size: " + repr(os.path.getsize(dest)),xbmc.LOGNOTICE)
            else:
                koding.Text_Box("BoB Unleashed", "gif not found: " + url)
                return None
        xbmc.log("gif done: " + repr(dest),xbmc.LOGNOTICE)
        return dest
